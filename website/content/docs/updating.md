...
sudo vps-deployer update --version 0.3.2
...
It reads the installed version, then uses `--version` or `GET https://vps-deployer.centralstackhq.com/releases/latest.txt`. If those versions match, it stops unless you pass `--force`.
